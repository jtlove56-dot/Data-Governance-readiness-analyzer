...
@pytest.mark.parametrize(
    ...
    [
        name
        for name, field in AssessmentRequest.model_fields.items()
...
def test_blank_or_oversized_answers_are_rejected(field: str) -> None:
    for invalid in [" \n ", "x" * 2001]:
        assert (
            client.post("/assessments", json={**EXAMPLE, field: invalid}).status_code
            == 422
        )
...
def test_explicit_uncertainty_is_accepted_but_invalid_choices_are_not(
    ...
    assert (
        client.post("/assessments", json={**EXAMPLE, field: "unsure"}).status_code
        == 200
    )
...
def test_unknown_fields_and_schema_versions_are_rejected() -> None:
    for extra in [{"schema_version": "2.0"}, {"risk_score": 0}]:
        assert client.post("/assessments", json={**EXAMPLE, **extra}).status_code == 422
