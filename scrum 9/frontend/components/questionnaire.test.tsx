...
function fillStep(index: number, answers = example) {
  if (index === 0)
    for (const value of answers.data_types) {
      const type = dataTypes.find((item) => item.value === value)!;
      fireEvent.click(
        screen.getByRole("checkbox", { name: `${type.label} ${type.example}` }),
      );
    }
  for (const q of steps[index].questions) {
    if (q.show && !q.show(answers as Answers)) continue;
    if (q.type === "text")
      fireEvent.change(screen.getByRole("textbox", { name: q.label }), {
        target: { value: answers[q.key] },
      });
    else
      fireEvent.click(
        document.querySelector(
          `input[name="${q.key}"][value="${answers[q.key]}"]`,
        )!,
      );
  }
}
...
describe("Questionnaire", () => {
  it("blocks empty steps and requires details when Other is selected", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "What data is involved?" }),
    ).toBeInTheDocument();
  });
  ...
  it("keeps multiple selections and text when navigating back", () => {
    ...
    expect(
      screen.getByRole("textbox", {
        name: "Which people are involved or affected?",
      }),
    ).toHaveValue(example.people);
  });
  ...
  it("cannot jump to review after clearing a previously completed required answer", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Who is involved?" }),
    ).toBeInTheDocument();
  });
  ...
  it("submits the exact shared backend contract to the configured URL", async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValue({ ok: true, json: async () => validatedReceipt });
    ...
    expect(fetchMock.mock.calls[0][0]).toBe(
      "https://api.example.test/assessments",
    );
  ...
  it("rejects a receipt that contradicts the confirmation message", async () => {
    ...
    expect(
      screen.queryByText("Your answers passed validation."),
