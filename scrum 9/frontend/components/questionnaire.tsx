...
const unexpectedResponseMessage =
  "The service returned an unexpected response. Your answers are still here; please try again.";
...
async function readAssessmentReceipt(
  response: Response,
...
export function Questionnaire() {
  ...
  async function submit() {
    ...
    try {
      ...
      const result = await readAssessmentReceipt(response);
      ...
      setFailure(
        error instanceof Error &&
          ...
          ? error.message
          : "We could not reach the service. Your answers are still here; please try again.",
      );
    } finally {
  ...
  function renderQuestion(question: Question) {
    ...
    return (
      <fieldset className="question" key={key}>
        <legend>
          {question.label} <span className="required">(required)</span>
        </legend>
        ...
        {question.type === "text" ? (
          ...
          <div className="choices" id={key} tabIndex={-1}>
            {(Object.keys(choiceLabels) as Choice[]).map((value) => (
        ...
        <aside className="sidebar">
          ...
          <nav aria-label="Questionnaire steps">
            <ol className="step-list">
              {[...steps.map((item) => item.title), "Review"].map(
                (title, index) => (
                  <li key={title}>
                    <button
                      type="button"
                      ...
                      onClick={() =>
                        index <= step ? navigate(index) : goForward(index)
                      }
                    >
          ...
          <div className="privacy-note">
            <strong>Describe the data.</strong>
            <p>
              Use examples and roles. Do not enter actual personal records,
          ...
          {receipt ? (
            <div className="confirmation" role="status">
              ...
              <h2 ref={heading} tabIndex={-1}>
                Your answers passed validation.
              </h2>
              <p>
                No risk score has been calculated. Your answers have not been
                saved to a database.
              </p>
              <p className="receipt">
                Submission reference: <span>{receipt}</span>
              </p>
              <button
                className="primary"
                type="button"
            ...
            <>
              <div className="form-heading">
                ...
                <h2 ref={heading} tabIndex={-1}>
                  {step < 4 ? steps[step].heading : "Review your answers"}
                </h2>
                <p>
                  {step < 4
                    ? steps[step].description
                    : "Check the details before submitting your questionnaire."}
                </p>
              </div>
                  ...
                  {step === 0 && (
                    ...
                    >
                      ...
                      <div className="data-grid">
                        {dataTypes.map((item) => (
                          <label
                            className={`data-option ${answers.data_types.includes(item.value) ? "selected" : ""}`}
                            key={item.value}
                          >
                            <input
                              type="checkbox"
                              value={item.value}
                              checked={answers.data_types.includes(item.value)}
                              aria-invalid={Boolean(errors.data_types)}
                              aria-describedby={
                                errors.data_types
                                  ? "data_types-error"
                                  : undefined
                              ...
                              onChange={(event) => {
                                ...
                                setAnswers((previous) => ({
                                  ...previous,
                                  data_types: checked
                                    ? [...previous.data_types, item.value]
                                    : previous.data_types.filter(
                      ...
                      {errors.data_types && (
                        <p className="field-error" id="data_types-error">
                          {errors.data_types}
                        </p>
                      )}
                    </fieldset>
                  ...
                  {step === 4 && (
                    <>
                      {steps.map((section, index) => (
                        <section className="review-section" key={section.title}>
                          ...
                          <dl>
                            {index === 0 && (
                              <div>
                                ...
                                <dd>
                                  {dataTypes
                                    .filter((item) =>
                                      answers.data_types.includes(item.value),
                                    )
                                    .map((item) => item.label)
                                    .join(", ")}
                                </dd>
                            ...
                            {section.questions
                              .filter((q) => !q.show || q.show(answers))
                              .map((q) => (
                                <div key={q.key}>
                                  <dt>{q.label}</dt>
                                  <dd>
                                    {q.type === "choice"
                                      ? choiceLabels[answers[q.key] as Choice]
                                      : answers[q.key]}
                                  </dd>
                                </div>
                              ))}
                      ...
                      <p className="submission-note">
                        Submitting checks your answers. Risk scoring and
                        database storage are not yet available. Answers remain
                        available while this page is open; refreshing or closing
                        it clears them.
                      </p>
                    </>
                  )}
              ...
              <p className="draft-note">
                Answers stay with you as you move between steps.
              </p>
            </>
          )}
        </section>
